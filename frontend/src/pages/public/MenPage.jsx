...
const MEN_CATEGORIES = ['All Men', 'Night Wear', 'Shirts', 'Trousers', 'Ethnic Wear'];

const MenPage = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedSubCat, setSelectedSubCat] = useState('All Men');
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const menItems = all.filter((p) =>
          ['Night Wear', 'Shirts', 'Trousers', 'Ethnic Wear'].includes(p.category)
        );
  ...
  const filteredProducts = products
    .filter((p) => {
      if (selectedSubCat === 'All Men') return true;
    ...
    <div className="bg-slate-50 min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="border-b border-slate-200 pb-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <div className="text-xs font-bold font-mono text-slate-500 uppercase tracking-widest flex items-center space-x-1.5">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Comfort Essentials</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mt-1">
              Men's Night Wear & Daily Casuals
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Comfortable cotton nightwear, track pants, loungewear, and daily casual shirts from our showroom.
            </p>
          </div>
        ...
        {/* Grid */}
        {loading ? (
          <LoadingSpinner message="Loading Men's Readymade Garments..." />
