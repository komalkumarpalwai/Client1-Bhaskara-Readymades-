...
const KidsPage = () => {
  ...
  const [selectedSubCat, setSelectedSubCat] = useState('All Kids');
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const kidsItems = all.filter((p) =>
          ['Boys Wear', 'Girls Wear'].includes(p.category)
        );
  ...
  const filteredProducts = products
    .filter((p) => {
      if (selectedSubCat === 'All Kids') return true;
    ...
    <div className="bg-slate-50 min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="border-b border-slate-200 pb-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <div className="text-xs font-bold font-mono text-purple-700 uppercase tracking-widest flex items-center space-x-1.5">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Kids & Juniors Readymade Collection</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mt-1">
              Kids' Readymade Wear
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Vibrant Boys' shirts, suits & Girls' frocks and festive wear from our Ganapavaram showroom.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-sm text-slate-500 bg-white border border-slate-200 rounded-2xl space-y-3">
            ...
            <Link to="/shop" className="text-xs font-bold text-purple-700 underline">
              Browse All Products
            </Link>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {paginatedProducts.map((product) => (
                <div
                  key={product.id}
                  className="bg-white border border-slate-200 hover:border-purple-400 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden group"
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      <Link to={`/products/${product.id}`}>
                        <h3 className="text-sm font-bold text-slate-900 group-hover:text-purple-700 line-clamp-2 transition leading-snug">
                          {product.name}
                        </h3>
                      </Link>
                      <p className="text-xs text-slate-500 line-clamp-2 mt-1.5 leading-relaxed">
                        {product.description || `Comfortable and durable ${product.category} for children.`}
                      </p>
                    </div>
